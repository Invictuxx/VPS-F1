...
RECORDING_DURATION = int(
    os.environ.get("RECORDING_DURATION", "30")
)

UPLOAD_RETRIES = int(
    os.environ.get("UPLOAD_RETRIES", "5")
)
...
CUSTOM_FILE_NAME = os.environ.get(
    "CUSTOM_FILE_NAME",
    ""
).strip()
...
def send_telegram(message):
    ...
    try:
        ...
        print(
            "AVISO: no se pudo enviar mensaje a Telegram: "
            f"{error}"
        )
...
def get_m3u8_url():
    ...
    if not STREAM_PAGE_URL:
        raise RuntimeError(
            "La variable STREAM_PAGE_URL no está configurada."
        )
    ...
    try:
        ...
        if array_match:
            ...
            if k_match:
                ...
                func1_match = re.search(
                    r'function\s+'
                    + re.escape(func1_name)
                    + r'\(\)\s*\{\s*'
                    r'return\s+(\d+)\s*;\s*\}',
                    html
                )

                func2_match = re.search(
                    r'function\s+'
                    + re.escape(func2_name)
                    + r'\(\)\s*\{\s*'
                    r'return\s+(\d+)\s*;\s*\}',
                    html
                )
                ...
                if func1_match and func2_match:
                    k_val = (
                        int(func1_match.group(1))
                        + int(func2_match.group(1))
                    )
                    ...
                    if ".m3u8" in decoded_url:
                        ...
                        print(
                            "¡URL desofuscada con éxito "
                            "mediante ingeniería inversa!"
                        )

    except Exception as error:
        print(
            "Advertencia: falló el intento "
            f"de desofuscación: {error}"
        )
    ...
    if not m3u8_url:
        raise RuntimeError(
            "No se encontró la URL .m3u8 en el HTML."
        )

    m3u8_url = javascript_unescape(
        m3u8_url
    ).strip()

    print(
        f"M3U8 Extraído: {m3u8_url}"
    )
...
def record_stream(
    ...
    if killed_for_timeout:
        raise RuntimeError(
            "FFmpeg no terminó tras "
            f"{max_seconds} segundos."
        )
...
def upload_to_filester(filename):
    ...
    raise RuntimeError(
        ...
        f"tras {UPLOAD_RETRIES} intentos. "
...
def main():
    ...
    try:
        ...
        success, url = upload_to_filester(
            video_file
        )
        ...
        if url:
            ...
            send_telegram(
                "✅ **Proceso completado**\n\n"
                "Subida correcta, pero Filester "
                "no devolvió un enlace."
            )
